...
CREATE PROC	p_Create_Funcionario(@num_ID int, @password varchar(50), @nome varchar(50), @loja_code int)
AS
	BEGIN TRY
		BEGIN TRANSACTION
			INSERT INTO SPORT_SHOP.[Utilizador](nome, num_ID) VALUES (@nome, @num_ID)
			INSERT INTO SPORT_SHOP.[Funcionario]([password], num_ID, loja_code) VALUES (@password, @num_ID, @loja_code)
		COMMIT
	END TRY
	BEGIN CATCH
		ROLLBACK
...
CREATE PROC p_Update_Stock(@produto_ID int, @quantidade_Loja int, @quantidade_Armazem int)
AS
	DECLARE @existe_Loja int
	DECLARE @existe_Armazem int
    BEGIN TRY
		BEGIN TRANSACTION
			SELECT @existe_Loja = quantidade_Loja, @existe_Armazem = quantidade_Armazem FROM SPORT_SHOP.[Stock] WHERE produto_ID = @produto_ID
			UPDATE SPORT_SHOP.[Stock] SET quantidade_Loja = @existe_Loja + @quantidade_Loja, quantidade_Armazem = @existe_Armazem + @quantidade_Armazem WHERE produto_ID = @produto_ID
		COMMIT
	END TRY
	BEGIN CATCH
		ROLLBACK
...
CREATE PROC p_retiraStock(@produto_ID int, @loja_code int, @quantidade int)
AS
	DECLARE @soja int
	DECLARE @sarm int
	BEGIN TRY
	BEGIN TRANSACTION
		SELECT @soja = quantidade_Loja, @sarm = quantidade_Armazem FROM SPORT_SHOP.Stock WHERE produto_ID = @produto_ID AND loja_code = @loja_code
		IF @sarm > @quantidade
		BEGIN
			UPDATE SPORT_SHOP.Stock SET quantidade_Armazem = @sarm - @quantidade WHERE produto_ID = @produto_ID AND loja_code = @loja_code
		END
		ELSE
		BEGIN
			SET @quantidade = @quantidade - @sarm
			UPDATE SPORT_SHOP.Stock SET quantidade_Loja = @soja - @quantidade, quantidade_Armazem = 0 WHERE produto_ID = @produto_ID AND loja_code = @loja_code
		END
	COMMIT
	END TRY
	BEGIN CATCH
